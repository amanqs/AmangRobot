...
def get_user_list(__init__, key):
    with open(f"{os.getcwd()}/AmangRobot/{__init__}", "r") as json_file:
        return json.load(json_file)[key]

# enable logging
FORMAT = "[AmangRobot] %(message)s"
logging.basicConfig(
...
LOGGER = logging.getLogger('[AmangRobot]')
LOGGER.info("Amang is starting. | An Kennedy Project Parts. | Licensed under GPLv3.")
...
if ENV:
    ...
    MONGO_DB = os.environ.get("MONGO_DB", "Amang")
    ...
    from AmangRobot.config import Development as Config
...
from AmangRobot.modules.helper_funcs.handlers import (
