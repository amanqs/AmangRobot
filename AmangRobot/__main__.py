...
import sys
import traceback
import AmangRobot.modules.sql.users_sql as sql
...
from AmangRobot.modules import ALL_MODULES
from AmangRobot.modules.helper_funcs.chat_status import is_user_admin
from AmangRobot.modules.helper_funcs.misc import paginate_modules
...
buttons = [
    ...
    [
        ...
        InlineKeyboardButton(text="❓ Help Music", callback_data="amang_"),
    ],
    [
        InlineKeyboardButton(
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module(f"AmangRobot.modules.{module_name}")
...
def amang_about_callback(update, context):
    query = update.callback_query
    ...
    elif query.data == "amang_support":
        query.message.edit_text(
            text="*๏ Amang support chats*"
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(
        amang_about_callback, pattern=r"amang_", run_async=True
    )
