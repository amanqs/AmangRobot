...
from AmangRobot import dispatcher
from AmangRobot.modules.helper_funcs.handlers import CMD_STARTERS, SpamChecker
from AmangRobot.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from AmangRobot.modules.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        args = context.args
        if len(args) >= 1:
            disable_module = "AmangRobot.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        args = context.args
        if len(args) >= 1:
            enable_module = "AmangRobot.modules." + args[0].rsplit(".", 1)[0]
